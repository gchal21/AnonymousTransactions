...
library Poseidon2 {
  ...
  function poseidon(bytes32[2] memory input) public pure returns (bytes32) {
        uint256[2] memory fieldInputs;
        fieldInputs[0] = uint256(input[0]);
        fieldInputs[1] = uint256(input[1]);
        return bytes32(poseidonConverted(fieldInputs));
  }
  ...
  function poseidonConverted(uint[2] memory) public pure returns (uint) {
    assembly {
